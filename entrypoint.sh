...
if [ -n "$LITESTREAM_S3_BUCKET" ]; then
  export CLAUDE_GATE_DB_PATH="${CLAUDE_GATE_DB_PATH:-/data/claude-gate.db}"
  litestream restore -config /etc/litestream.yml -if-db-not-exists -if-replica-exists "$CLAUDE_GATE_DB_PATH"
